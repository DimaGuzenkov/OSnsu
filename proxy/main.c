...
static void *fetcher_thread(void *arg) {
        ...
        if (header_end) {
            ...
            if (header_len < sizeof(headers)) {
                ...
                if (content_length <= 0) {
                    ...
                    e->cacheable = 0;
                    // Отправляем оставшиеся данные клиентам напрямую
                    pthread_mutex_lock(&e->lock);
                    pthread_cond_broadcast(&e->cond);
                    pthread_mutex_unlock(&e->lock);
                    // Продолжаем читать чтобы освободить сокет
                    while ((r = recv(sock, buf, sizeof(buf), 0)) > 0) {
                        // Просто читаем чтобы очистить буфер
                    }
                    close(sock);
                    pthread_mutex_lock(&e->lock);
                    e->complete = 1;
                ...
                log_fetcher(e->url, "Headers too large, cannot cache");
                e->cacheable = 0;
            }
        }
    }
    ...
    if (e->cacheable) {
        ...
        // Если файл не кэшируемый, просто читаем до конца
        while ((r = recv(sock, buf, sizeof(buf), 0)) > 0) {
            // Просто читаем чтобы очистить буфер
        }
    }
